class VencimientosController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @vencimiento.update(vencimiento_params)
        format.html { redirect_to @vencimiento, notice: 'Vencimiento actualizado con exito.' }
  ...
  def destroy
    @vencimiento.destroy
    respond_to do |format|
      format.html { redirect_to vencimientos_url, notice: 'Vencimiento eliminado con exito.' }
  ...
  private
    ...
    def vencimiento_params
      params.require(:vencimiento).permit(:company_Id, :employee_Id, :study_Id, :start_date, :end_date, :status, :comments)
    end
end
